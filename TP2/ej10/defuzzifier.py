...
class Defuzzifier:
    ...
    def defuzzify(self,
        ...
        # Aplicar SOM (Smallest of Maxima) para "CERRAR"
        if self.method == 'SOM' and 'CERRAR' in agg_strength:
            max_mu = max(mus)
            max_points = [x for x, mu in zip(xs, mus) if abs(mu - max_mu) < 1e-9]
            return min(max_points)

        # Aplicar LOM (Largest of Maxima) para "ABRIR"
        if self.method == 'LOM' and 'ABRIR' in agg_strength:
            max_mu = max(mus)
            max_points = [x for x, mu in zip(xs, mus) if abs(mu - max_mu) < 1e-9]
            return max(max_points)

        # Localizadores de máximos para MOM (Mean of Maxima) o cualquier otro método
        max_mu = max(mus)
        if max_mu == 0:
