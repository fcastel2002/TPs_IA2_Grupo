...
class Videojuego:
    def __init__(self):
        ...
        self.population_number = 200
        
        self.population = self.generar_poblacion(self.population_number)
        
        self.nombre_carpeta_datos = "datos_buenas_poblaciones"
    ...
    def correr_videojuego(self):
        ...
        if self.playMode == 'x' or 'l':
            # Crear el selector de menú
            menu = MenuSelector(
                window_size=(SCREEN_WIDTH, SCREEN_HEIGHT),
                colors=self.custom_colors,  # Opcional, usa colores por defecto si no se especifica
                folder_name=self.nombre_carpeta_datos
            )
            ...
            if self.playMode == 'l':
                # Asegurarse de seleccionar el mejor dinosaurio según el atributo score
                best_dino = max(self.population, key=lambda d: d.score)
                self.population = [best_dino]  # Solo el mejor dinosaurio de la población
            
            
        while self.run:
            if self.playMode == 'm' or self.playMode == 'c' or self.playMode == 'a':
                self.player.resetStatus()
            elif self.playMode == 'x':
                updateNetwork(self.population, self.generaciones)
            elif self.playMode == 'l':
                self.population[0].resetStatus()
    ...
    def gameScreen(self):
        ...
        while self.run:
            ...
            '''Generación de obstáculos inicial'''
            ...
            '''Generación de obstáculos modificada'''        
            # if len(self.obstacles) == 0:
            #     # 50% chance for cactus, 50% for bird
            #     if random.random() < 0.5:
            #         # Cactus: 50% small, 50% large
            #         if random.random() < 0.5:
            #             self.obstacles.append(SmallCactus(SCREEN_WIDTH, self.game_speed, self.obstacles))
            #         else:
            #             self.obstacles.append(LargeCactus(SCREEN_WIDTH, self.game_speed, self.obstacles))
