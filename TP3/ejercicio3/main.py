...
MAX_GAME_SPEED      = 70     # velocidad tope que consideramos
generation = 1
bestScore = 0
playMode = "X"
    ...
    while run:
        ...
        if playMode == 'm' or playMode == 'c':
            ...
            for dino in population:
                ...
                # Solo procesamos el obstáculo más cercano
                if not obstacles:
                    continue
                obstacle = obstacles[0]

                # 1) Calcular features
                distancia = (obstacle.rect.x - dino.dino_rect.x) / SCREEN_WIDTH
                tipo      = 0 if isinstance(obstacle, (SmallCactus, LargeCactus)) else 1
                ancho     = obstacle.rect.width / MAX_OBSTACLE_WIDTH
                alto      = obstacle.rect.height / MAX_OBSTACLE_HEIGHT
                y_dino    = dino.dino_rect.y / SCREEN_HEIGHT
                velocidad = game_speed / MAX_GAME_SPEED

                # 2) Llamar a think() + act()
                # 1) Obtengo los logits de la red
                logits = dino.think([distancia, tipo, ancho, alto, y_dino, velocidad])
                # 2) Convierto esos logits en una acción string
                accion = dino.act(logits)
